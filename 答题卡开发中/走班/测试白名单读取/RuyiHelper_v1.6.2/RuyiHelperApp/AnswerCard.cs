    ...
    public class AnswerCard
    {
        ...
        public void Message()
        {
           ...
           // AnswersCollection.HX_RemovefromWhitelist(null);
            ...
            //获取白名单中的学生列表添加到Global.stuKQlist变量下
            //int flag = AnswersCollection.HX_GetWhitelist(arrStu);
            //ArrayList al = new ArrayList();
            //string date = DateTime.Now.ToString("yyyyMMddHHmmss");
            //if(flag == 0)
            //{
            //    int len = arrStu.Length;
            //    for (var i = 0; i < len;i++ ) 
            //    {
            //        if(arrStu[i] != 0)
            //        {
            //            if (Global.stuKQlist == "")
            //            {
            //                Global.stuKQlist = arrStu[i] + ":" + date;
            //            }
            //            else
            //            {
            //                Global.stuKQlist += ";" + arrStu[i] + ":" + date;
        ...
        //public static ulong[] stuList = new ulong[100];
        private void Theout(object sender, System.Timers.ElapsedEventArgs e)
        {          
            ulong[] stuList = new ulong[100];
            int a = AnswersCollection.HX_GetWhitelist(stuList);
            if(a < 0)
            {
                //t.Enabled = false;
                //t.Enabled = true;
        ...
        public void CallbackFun(int iDevice, AnswersCollection.CALLBACK_MSG msg, int param1, string param2)
        {
            ...
            //考勤注册
            if (msg == AnswersCollection.CALLBACK_MSG.MSG_REGISTER_DATA)
            {                
                string[] t = param2.ToString().Split(new char[] { '"' });
                string date = DateTime.Now.ToString("yyyyMMddHHmmss");
                if (Global.isLessonOff())
                {
                    if (Global.stuKQlist.IndexOf(t[7]) < 0)
                    {
                        if (Global.stuKQlist == "")
                        {
                            Global.stuKQlist = t[7] + ":" + date;
                        }
                        else
                        {
                            Global.stuKQlist += ";" + t[7] + ":" + date;
                ...
                else 
                {
                    ...
                    if (Global.stuPadon.IndexOf(t[7]) < 0)
                    {
                        Common.uploadPerKQinfor(schooid, classid, Global.lessonindex, Reader, date, t[7]);
                        if (Global.stuPadon == "")
                        {
                            Global.stuPadon = t[7] + ":" + date;
                        }
                        else
                        {
                            Global.stuPadon += ";" + t[7] + ":" + date;
