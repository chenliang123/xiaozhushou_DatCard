    ...
    class RueSqlite
    {
        ...
        public List<User> getStudentlist(string classid)
        {
            ...
            while (reader.Read())
            {
                ...
                u.cardid = reader["cardid"].ToString();
        ...
        //从本地数据库中获取对应卡号的学生信息
        public User getCardlist(string cardid)
        {
            //List<User> ulist = new List<User>();
            string sql = "select * from edu_user where cardid='" + cardid + "' and type=2";
            SQLiteCommand command = new SQLiteCommand(sql, m_dbConnection);
            SQLiteDataReader reader = command.ExecuteReader();
            User u = new User();
            while (reader.Read())
        ...
        public int addUser(List<User> ulist)
        {
            ...
            string sqlSchma = "insert into edu_user (id,account,phone,pwd,name,classid,schoolid,type,seat,classname,courseid,coursename,imageurl,cardid) values (P1,'P2','P3','P4','P5',P6,P7,P8,'P9','PA','PB','PC','PD','PE')";
            ...
            try
            {
                foreach(User u in ulist)
                {
                    ...
                    sql = sql.Replace("PA", "").Replace("PB", u.courseid + "").Replace("PC", u.coursename).Replace("PD", u.imageurl).Replace("PE", u.cardid);
        ...
        private int addUser(User u)
        {
            ...
            int ret = 0;
            string sql = "insert into edu_user (id,account,phone,pwd,name,classid,schoolid,type,seat,classname,courseid,coursename,imageurl,cardid) values (P1,'P2','P3','P4','P5',P6,P7,P8,'P9','PA','PB','PC','PD','PE')";
            sql = sql.Replace("P1", u.id + "").Replace("P2", u.account).Replace("P3", u.phone).Replace("P4", u.pwd).Replace("P5", u.name);
            sql = sql.Replace("P6", u.classid + "").Replace("P7", u.schoolid + "").Replace("P8", u.type + "").Replace("P9", u.seat);
            sql = sql.Replace("PA", "").Replace("PB", u.courseid + "").Replace("PC", u.coursename).Replace("PD", u.imageurl).Replace("PE", u.cardid);
            try{
        ...
        private static void createTable_User()
        {
            ...
            string sql = "create table edu_user (id int,account varchar(50) ,phone varchar(50),pwd varchar(50),name varchar(50),cardid varchar(45),classid int,classname varchar(20),schoolid int,type int,seat varchar(10),courseid int, coursename varchar(20), imageurl varchar(256))";
            try
            {
                SQLiteCommand command = new SQLiteCommand(sql, m_dbConnection);
