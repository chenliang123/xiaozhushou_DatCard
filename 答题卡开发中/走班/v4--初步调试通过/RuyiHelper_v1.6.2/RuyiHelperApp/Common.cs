...
namespace RueHelper
{
    ...
    class Common
    {
        ...
        //上传白名单中学生考勤信息到后台
        public static void uploadAllKQinfor(int schoolid , int classid ,string lessonnum,string hdid,string msg)
        {
            ////异步同步到云服务
            string data = "schoolid=" + schoolid;
            data += "&classid=" + classid;
            data += "&lessonnum=" + lessonnum;
            data += "&hdid=" + hdid;
            data += "&msg=" + msg;
            Log.Info(data);
            doPostDataAsync("addAttendanceOfClass", data);
            return;
        }

        //上传白名单中学生考勤信息到后台
        public static void uploadPerKQinfor(int schoolid,int classid,string lessonnum, string hdid,string clocktime, string cardid)
        {
            ////异步同步到云服务
            string data = "schoolid=" + schoolid;
            data += "&classid=" + classid;
            ...
            data += "&cardid=" + cardid;
            Log.Info(data);
            doPostAsync("addAttendanceOfStudent", data);
            return;
        }
        ...
        {
            string jsonText = doPost("newClassinfo.get","");
            return jsonText;
        }
        /// <summary>
        /// getClassInfo
        ...
        /// getClassInfo
        /// </summary>
        public static string getnewSchoolinfo(int schoolid, string authcode)
        {
        ...
        /// <summary>
        /// getClassroomCourseTable
        /// </summary>
        public static string getClassroomCourseTable(int schoolid,int hdid,string day)
        {
            string ret = doPost("getClassroomCourseTable", "schoolid=" + schoolid + "&hdid=" + hdid + "&day=" + day);
            if (ret.Length > 0)
            {
                JObject jo = (JObject)JsonConvert.DeserializeObject(ret);
                string _data = jo["data"].ToString();
        ...
        //直接上传数据到后台
        public static void doPostDataAsync(string action, string data)
        {
            Thread th = new Thread(delegate()
            {
                string ret = doPostData(action, data);
            });
            th.Start();
        }
        public static string doPostData(string action, string data)
        {
